	...
	$(function(){
		...
		$("#import_btn").click(function() {
			...
			if(filename==""){
				...
				url = "StudentServlet?opType=importFile";
				importFile();
			}
	    });
		...
		$("#export_btn").click(function() {
			$.messager.confirm('确认对话框', '确认下载?', function(r){
				if (r){
					url = "StudentServlet?opType=exportFile&filename=student.xls";
					exportFile();
					/* 
		...
		$("#dg").datagrid({
			...
			columns:[[
				{field:'',checkbox:true},
				{field:'id',width:100,align:'center',title:'编号',sortable:true,hidden:false},
				{field:'name',width:100,align:'center',title:'姓名',sortable:true},
				{field:'username',width:100,align:'center',title:'帐号',sortable:true},
				{field:'password',width:100,align:'center',title:'密码',sortable:true},
				{field:'sex',width:100,align:'center',title:'性别',sortable:true,formatter:function(value,row,index){
					if(value==1){
						return "男";
					}else{
						return "女";
				...
				{field:'age',width:100,align:'center',title:'年龄',sortable:true},
				{field:'birthday',width:100,align:'center',title:'生日',sortable:true},
				{field:'createTime',width:100,align:'center',title:'创建时间',sortable:true}
			]],
			toolbar:[{
				text:"新增",
				iconCls:"icon-add",
		...
		<form id="Form" method="post">
			<table style="margin: 0 auto;">
				<tr>
					...
					<td><input name="name" class="easyui-textbox" data-options="required:true,iconCls:'icon-man'"></td>
				</tr>
				<tr>
					<td class="right">帐号：</td>
					<td><input name="username" class="easyui-textbox" data-options="required:true,iconCls:'icon-man'"></td>
				</tr>
				<tr>
					<td class="right">密码：</td>
					<td><input type="password" id="pwd" name="password" class="easyui-textbox" data-options="required:true,iconCls:'icon-lock',validType:'length[6,20]'"></td>
				</tr>
				<tr>
					<td class="right">确认密码：</td>
					<td><input type="password" id="rpwd" name="rpwd" class="easyui-textbox" data-options="required:true,iconCls:'icon-lock',validType:'equals[\'#pwd\']'"></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><input name="age" class="easyui-numberbox" data-options="required:true,validType:'age[0,99]'"></td>
				</tr>
				<tr>
					<td class="right">所属班级：</td>
					<td><input name="deptId" class="easyui-combobox" style="width:100px;"  data-options="editable:false,required:true,valueField:'id',textField:'name',url:'student/queryClasses'" /></td>
				</tr>
				<tr>
					<td class="right">出生日期：</td>
					<td><input name="birthday" class="easyui-datebox" data-options="required:true,editable:false"></td>
				</tr>
			</table>
		</form>
	</div>
